...
public class UserProgramCode
{
    ...
    {
        // Rule 1: Validate input1 (only alphabets and spaces)
        ...
        // Rule 2: Validate input2 (positive number)
        if (input2 <= 0)
            return "-2";

        // Rule 3: Validate input3 (must be special character)
        if (char.IsLetterOrDigit(input3))
            return "-3";

        string[] words = input1.Split(' ');

        // Replace nth word (1-based index)
